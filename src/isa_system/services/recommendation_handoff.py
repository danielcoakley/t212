...
def build_recommendation_handoff(
    ...
    rows = [_handoff_row(item) for item in response.recommendations]
...
def _handoff_row(item: TradeRecommendation) -> RecommendationHandoffRow:
    ...
    if "CATALYST_BLACKOUT" in item.risk_flags:
        return _blocked_event_row(item, blockers)
    ...
    if item.action == RecommendationAction.REVIEW_BUY:
        ...
        return RecommendationHandoffRow(
            ...
            blockers=[*blockers, "BROKER_INSTRUMENT_VALIDATION_REQUIRED"],
            next_step=(
...
def _blocked_event_row(
    item: TradeRecommendation, blockers: Sequence[str]
