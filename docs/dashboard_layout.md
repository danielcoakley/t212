...
| Recommendations | Holdings action queue, config-backed wider-market candidate feed, evidence context table, action label, confidence, deterministic rationale, optional OpenAI/LLM rationale, blockers, source freshness, rebalance hand-off status, broker instrument validation | Bring current positions and discovery candidates into a single review surface without creating orders | Edit scan universe config, mark reviewed, ignore candidate, exclude ticker, validate broker instrument, send eligible reviewed idea to rebalance preview |
...
| Wider-market scan feed | `configs/universe.example.yaml`, convenience EOD bars, convenience fundamentals, cached symbol universe | Surface discovery candidates beyond current holdings for operator review | Label as prototype discovery, show provider caveats, and do not treat as official PIT or broker-accessible truth. |
...
| Evidence context table | Valuation metrics, technical indicators, sentiment/news counts, catalyst counts, source warnings | Show the raw fields behind each recommendation score so the operator can challenge the action label | Missing or stale evidence remains visible and does not become an automatic trade instruction. |
...
| Review hand-off | Recommendation output, broker-holding status, blockers, rebalance preview state | Show which reviewed ideas can move toward preview sizing and which require broker instrument, ISA eligibility, liquidity, or official-source validation | Human review is required and hand-off still produces preview-only output until paper or guarded live controls pass. |
| Broker instrument validation | Trading 212 instrument metadata, broker position symbols, recommendation candidates | Confirm existing holdings and identify whether wider-market candidates have a plausible broker ticker or ambiguous mapping | Treat metadata matches as symbol validation only; still require ISA, liquidity, official-source, and operator review. |
...
| 4. Recommendation MVP | Done, then expand | Holdings action queue, config-backed wider-market scan feed, deterministic rationale, optional OpenAI/LLM rationale, blocker strip, broker instrument validation, review hand-off | Operator can review suggested actions and discovery candidates without any direct order path; market-scan buys show validation gates before preview. |
