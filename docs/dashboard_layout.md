...
| 4. Recommendation MVP | Done, then expand | Holdings action queue, wider-market scan feed, deterministic rationale, optional OpenAI/LLM rationale, blocker strip, review hand-off | Operator can review suggested actions and discovery candidates without any direct order path; market-scan buys show validation gates before preview. |
