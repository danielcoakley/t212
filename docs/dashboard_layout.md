...
| Catalysts | Earnings, filings, RNS/NSM validations, blackout windows, event vetoes | Avoid buying near unmanaged events or unverified announcements | Open source reference, apply event veto, review blackout |
| Rebalance preview | Current vs target, HOLD preview, target drift, costs, SDRT, FX, slippage, warnings, safety gates | Review proposed changes before paper or live submission | Export preview, paper submit, reject batch, arm live only outside this page |
| Factors | Factor z-scores, sector-neutral adjustments, missing data diagnostics, config hash | Explain ranks and missing-data policy | Review strategy config, compare factor contribution, mark exclusions |
...
| Audit and status | Audit chain, idempotency keys, heartbeats, broker reconciliation, run artefacts | Support replayability and incident review | Filter logs, reconcile, trigger kill switch, export evidence |
...
| 3. Valuation page | In progress | Valuation summary, valuation multiples, technical indicators, event/news/sentiment context, source provenance, missing-data warnings | A holding or candidate can be assessed from PIT valuation context before rebalance. |
| 4. Paper workflow | In progress | Rebalance preview, target weights, cost model, event vetoes, paper submit, simulated fills | Paper batches can be reviewed, submitted, reconciled, and audited. |
| 5. Official data and factors | Planned | PIT source status, factor attribution, config hash, stale data, missing-data policy | Factor ranks show source timing and cannot use future data. |
...
| 7. Audit evidence | In progress | Audit log, run artefacts, broker reconciliation, CI/smoke status | A reviewer can replay why a preview, paper fill, or blocked live action occurred. |
