...
| Management | Runtime mode, live arming, kill switch, broker status, cache window, provider configuration, safety checklist, blocked future capabilities | Give the operator one read-only control surface for setup and safety status | Confirm provider gaps, deep research availability, paper readiness, and live guardrails |
...
| 6. Management and safety status | In progress | Read-only Management page with runtime mode, live arming, kill switch, cache window, provider status, and safety checklist | Operator can see setup gaps and live guardrails without leaving the dashboard. |
