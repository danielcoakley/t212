...
| Rebalance preview | Current vs preview target, sleeve blend, drift chart, turnover, cost components, SDRT, PTM, FX, slippage, warnings, safety gates | Review proposed changes before paper or live submission | Export preview, paper submit, reject batch, arm live only outside this page |
...
| 4. Paper workflow | In progress | Preview-only sleeve targets, target drift, cost components, event vetoes, paper submit, simulated fills | Paper batches can be reviewed, submitted, reconciled, and audited. |
